...
void displayer(u16 gfxoffset)
{
    ...
    oamSet(0, player.x-camera.x, player.y-camera.y, 3, player.direction, 0, gfxoffset, 0);
}
...
{
    bgSetScroll(0,256-(player.x-camera.x-48),256-(player.y-camera.y-48));    
}
...
void player_state_0(void)
{
    ...
    if (player.tics == 0) player.animf = WALK1;
    player.tics = 1;
...
void player_state_1(void)
{
    ...
    if (player.tics > 4)
    {
        ...
        if (player.frame == 0) player.animf = WALK1;
        if (player.frame == 1) player.animf = WALK2;
        if (player.frame == 2) player.animf = WALK3;
        if (player.frame == 3) player.animf = WALK2;
        if (player.frame == 4) player.animf = WALK1;
        if (player.frame == 5) player.animf = WALK0;
    }
    if (pad & KEY_LEFT)
    {
        stillwalking = 1;
...
void camera_state_0(void)
{
    /* Check camera position vs ideal target player position */
    if (player.x > IDEALX && (player.x - camera.x) != IDEALX) camera.state = 1;
    if (camera.x < 256 && (player.x - camera.x) != IDEALX) camera.state = 1;
    if (player.y > IDEALY && (player.y - camera.y) != IDEALY) camera.state = 1;
    if (camera.y < 256 && (player.y - camera.y) != IDEALY) camera.state = 1;
}

void camera_state_1(void)
{
    camera.x = player.x - IDEALX;
    camera.y = player.y - IDEALY;
    camera.state = 0;
}
...
1 - move
*/
        if (camera.state == 0) camera_state_0();
        if (camera.state == 1) camera_state_1();
}
